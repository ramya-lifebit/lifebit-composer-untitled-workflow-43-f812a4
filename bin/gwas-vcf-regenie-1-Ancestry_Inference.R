...
head(count_per_population)
write.csv( count_per_population, file = "count_per_population.csv")
...
for (i in 1:length(list_of_dataframes)) {
  ...
  write.table( list_of_dataframes[[i]]
            , sep = " "
            ,file = paste0( ancestry_label
                          , "_"
                          , ancestry_count
                          , "_"
                          , "InferredAncestry.txt"))
}
